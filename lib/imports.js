  ...
  root.find(j.ImportDeclaration).forEach((path) => {
    ...
    if (path.node.specifiers) {
      for (const spec of path.node.specifiers) {
        ...
        if (REMOVED_HOOKS[importedName]) {
          // The hook isn't exported from v3. Drop the specifier so
          // `npm install` doesn't fail with a missing-export error. The
          // call site will show a TS2304 (undefined identifier) which is
          // the right signal — accompanied by hooks.js's HEROUI-MIGRATE
          // comment explaining the compound replacement.
          if (localName) removedImportLocals.add(localName);
          recordWarning(
            ctx,
            ctx.filePath,
            spec.loc && spec.loc.start && spec.loc.start.line,
            'removed-hook-import',
            `'${importedName}' is removed from @heroui/react in v3. ${REMOVED_HOOKS[importedName]}`
          );
          attachLeadingComment(
            path.node,
            `'${importedName}' is removed from @heroui/react in v3. ${REMOVED_HOOKS[importedName]}`
