...
function patchPackageJson(repoRoot, opts) {
  ...
  if (dropFramerMotion) {
    const stillReferenced = sourceReferencesFramerMotion(repoRoot);
    if (!stillReferenced) {
...
function sourceReferencesFramerMotion(repoRoot) {
  const candidates = fastGlob.sync(
    ['**/*.{ts,tsx,js,jsx,mjs,cjs}'],
    {
      cwd: repoRoot,
      ignore: ['node_modules/**', 'dist/**', 'build/**', '.next/**', '.cache/**', 'coverage/**'],
      onlyFiles: true,
      absolute: true,
    }
  );
  for (const file of candidates) {
    try {
      ...
      if (/from\s+['"]framer-motion['"]/.test(c) || /require\(['"]framer-motion['"]\)/.test(c)) {
        return true;
      }
    } catch (_) { /* ignore */ }
  }
